...
export const recipe = createTRPCRouter({
  getAllRecipes: publicProcedure.query(async ({ ctx }): Promise<Recipe[]> => {
    return await ctx.db.query.recipes.findMany({
      orderBy: (recipes, { asc }) => [
        asc(sql<string>`CAST(${recipes.profession} as TEXT)`),
        asc(recipes.recipe),
      ],
    });
  }),
  ...
  getAllRecipesWithCharacters: publicProcedure.query(
    async ({ ctx }): Promise<RecipeWithCharacters[]> => {
      ...
      // Map the results to include characters as an array, filtering out ignored characters and those whose primary is ignored
