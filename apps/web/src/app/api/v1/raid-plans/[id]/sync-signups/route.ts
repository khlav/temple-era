...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!raidHelperEventId) {
      return NextResponse.json({ error: "Plan has no linked Raid Helper event" }, { status: 400 });
    }

    // 2. Fetch event from Raid Helper, resolve lastEventId if needed
    const initialResponse = await fetch(`${RAID_HELPER_API_BASE}/v4/events/${raidHelperEventId}`, {
    ...
    if (actualEventId !== raidHelperEventId) {
      const resolvedResponse = await fetch(`${RAID_HELPER_API_BASE}/v4/events/${actualEventId}`, {
        headers: { Authorization: env.RAID_HELPER_API_KEY },
      });
