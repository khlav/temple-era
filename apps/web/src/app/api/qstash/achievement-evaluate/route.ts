...
import { characters, raidLogAttendeeMap, raidLogs } from "~/server/db/schema";
import { verifyQstashRequest } from "~/server/services/qstash-verify";
import { evaluateAchievementsForFamily } from "~/server/services/achievement-rules";
...
function isAchievementEvaluateBody(value: unknown): value is AchievementEvaluateBody {
  ...
  return (
    ...
    (trigger === "raid_log_import" || trigger === "signup_link_resolved")
  );
}
...
async function resolvePrimaryCharacterIdsForRaid(raidId: number): Promise<number[]> {
  ...
  return rows.map((r) => r.primaryCharacterId);
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // evaluation for the rest of this raid's attendees — evaluateAchievementsForFamily
    // does not self-catch, so isolate each family's call.
    let newAwardsTotal = 0;
    let failures = 0;
    for (const primaryCharacterId of primaryCharacterIds) {
      try {
        const { newAwards } = await evaluateAchievementsForFamily(
          db,
          primaryCharacterId,
          new Date(),
        );
        ...
        logger.error(
          { err, raidId: payload.raidId, primaryCharacterId },
